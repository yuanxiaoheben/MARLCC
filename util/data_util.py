...
def load_video_features_activity(root, max_position_length):
    video_features = dict()
    with h5py.File(os.path.join(root, 'cmcs_features.hdf5'), 'r') as f:
        for vid in f:
            feature = f[vid][:]
            if max_position_length is None:
                video_features[vid] = feature
            else:
                new_feature = visual_feature_sampling(feature, max_num_clips=max_position_length)
                video_features[vid] = new_feature
    return video_features
