...
class CharadesProcessor:
    ...
    def convert(self, data_dir):
        ...
        val_data = load_lines(os.path.join(data_dir, 'charades_sta_val.txt'))
        ...
        val_set = self.process_data(val_data, charades, scope='val')
...
def qf_dataset_gen(data, vfeat_lens, word_dict, char_dict, max_pos_len, scope, neg_ratio = 1):
    dataset = dict()
    pair_set = set()
    for record in tqdm(data, total=len(data), desc='process {} data'.format(scope)):
        ...
        result = {'sample_id': record['sample_id'], 'vid': record['vid'], 's_time': record['s_time'],
                  ...
                  's_ind': int(s_ind), 'e_ind': int(e_ind), 
                  'v_len': vfeat_lens[vid], 'w_ids': word_ids,
                  'c_ids': char_ids, 'is_match':1}
        dataset[vid].append(result)
        pair_set.add((vid, record['sentence']))
    # add neg
    for vid in dataset:
        curr_pool = dataset[vid]
        neg_num = int(neg_ratio * len(curr_pool))
        for i in range(neg_num):
            curr_data = random.choice(data)
            while (vid, curr_data['sentence']) in pair_set:
                curr_data = random.choice(data)
            word_ids, char_ids = [], []
            for word in curr_data['words'][0:max_pos_len]:
                char_id = [char_dict[char] if char in char_dict else char_dict[UNK] for char in word]
                word_ids.append(word_id)
                char_ids.append(char_id)
            result = {'vid': vid, 'duration': curr_data['duration'], 'words': curr_data['words'],
                  'v_len': vfeat_lens[vid], 'w_ids': word_ids, 'c_ids': char_ids, 'is_match':0}
            curr_pool.append(result)
        dataset[vid] = curr_pool
    return dataset
...
def gen_or_load_dataset(configs):
    ...
    train_set = qf_dataset_gen(train_data, vfeat_lens, word_dict, char_dict, configs.max_pos_len, 'train')
    val_set = None if val_data is None else qf_dataset_gen(val_data, vfeat_lens, word_dict, char_dict,
                                                        configs.max_pos_len, 'val')
    test_set = qf_dataset_gen(test_data, vfeat_lens, word_dict, char_dict, configs.max_pos_len, 'test')
    # save dataset
    n_val = 0 if val_set is None else len(val_set)
    dataset = {'train_set': train_set, 'val_set': val_set, 'test_set': test_set,
               'n_train': len(train_set), 'n_val': n_val, 'n_words': len(word_dict),  'n_chars': len(char_dict), 'word_dict': word_dict,
               'char_dict': char_dict, 'n_test': len(test_set), 'word_vector': vectors}
